...
from utils.logger_setup_layer_stack import logger
import video_frames_extract
from extractor import visualise_vgg, visualise_resnet, visualise_vit_layer
...
def get_video_paths(network_name, video_type, compressed_type, videodata, i):
    ...
    if compressed_type == 'original':
        ...
        # NR: original
            ...
            video_name = os.path.splitext(os.path.basename(video_path))[0]
        sampled_frame_path = os.path.join(common_path, f'layer_stack', f'video_{str(i + 1)}')
...
def get_deep_feature(network_name, video_name, image_path, qp):
    ...
    if network_name == 'resnet50':
        ...
        name_model = 'vit_base'
        patch_size = 16
        # model = visualise_vit.VitGenerator(name_model, patch_size, device, evaluate=True, random=False, verbose=True)
        # frame_npy, _ = visualise_vit.process_video_frame(image_path, video_name, qp, model, patch_size, device)
...
if __name__ == '__main__':
    ...
    if video_type in valid_video_types:
        for i in range(len(videodata)):
            ...
            video_frames_extract.process_video(video_type, video_name, frame_interval, video_path, sampled_frame_path, video_width, video_height, pixfmt, framerate)
            ...
            for image_path in image_paths:
                logger.info(f"\n{image_path}")
                png_path, npy_path, frame_npy = get_deep_feature(network_name, video_name, image_path, qp)
                all_frame_activations.append(frame_npy)

            averaged_frame_npy = process_video_feature(all_frame_activations, network_name)
            dataset_name = video_type.replace('original_', '')
            output_npy_path = f'../features/{network_name}/{layer_name}/{dataset_name}/{compressed_type}/'
            ...
            np.save(output_npy_name, averaged_frame_npy)
