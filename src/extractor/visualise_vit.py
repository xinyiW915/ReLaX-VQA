...
def get_activation_npy(npy_path, video_name, frame_number, qp, fig_name, combined_name, attention):
    ...
    activations_dict = {}
    for i in range(n_heads):
        print(f'head_{i}', frame_number, f'qp_{qp}')
        # save activation features as pny
        # np.save(f'{npy_path}{fig_name}_head_{i + 1}.npy', attention[i])

        # save activation arrays to the dictionary
        activations_dict[i] = attention[i]
        # logger_setup.logger.debug(f'Shape of activation array: {attention[i].shape}')
        # print((f'Shape of activation array: {attention[i].shape}'))

    # save all activation arrays to Numpy file
    frame_activation_npy_path = f'../features/vit/{video_name}/frame_activation_{frame_number}_{combined_name}.npy'
    return activations_dict, frame_activation_npy_path
...
def process_video_frame(image_path, video_name, qp, model, patch_size, device):
    ...
    activations_dict, frame_activation_npy_path = visualize_predict(model, img, img_size, patch_size, device, png_path, npy_path, video_name, frame_number, qp, fig_name, combined_name)
    return activations_dict, frame_activation_npy_path
