...
def get_video_paths(network_name, video_type, compressed_type, videodata, i):
    ...
    if compressed_type == 'original':
        ...
        # NR: original
            ...
            video_name = os.path.splitext(os.path.basename(video_path))[0]
        sampled_frame_path = os.path.join(common_path, f'layer', f'video_{str(i + 1)}')
    ...
    if video_type in valid_video_types:
        for i in range(len(videodata)):
            ...
            for image_path in image_paths:
                logger.info(f"\n{image_path}")
                png_path, npy_path, frame_npy = get_deep_feature(network_name, video_name, image_path, qp, layer_name)
                all_frame_activations.append(frame_npy)

            averaged_frame_npy = process_video_feature(all_frame_activations, network_name)
            dataset_name = video_type.replace('original_', '').replace('encoded_', '')
