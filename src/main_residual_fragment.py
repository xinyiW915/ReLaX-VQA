...
def load_metadata(video_type, compressed_type, codec_name=None):
    ...
    if 'original' in compressed_type:
        ...
        # NR: original
            ...
            # return pd.read_csv(f'../metadata/{video_type.upper()}_metadata_residual_fragment.csv')
...
def get_video_paths(network_name, video_type, compressed_type, videodata, i):
    ...
    if compressed_type == 'original':
        ...
        # NR: original
            ...
            video_name = os.path.splitext(os.path.basename(video_path))[0]
        sampled_frame_path = os.path.join(common_path, f'residual_fragment', f'video_{str(i + 1)}')
...
def extract_important_patches(residual_frame, diff, patch_size=16, target_size=224, top_n=196):
    ...
    top_patches = zip(patch_idx[0][:top_n], patch_idx[1][:top_n])
...
def process_patches(original_path, residual_name, residual, patch_size, target_size, top_n):
    ...
    imp_patches = extract_important_patches(residual, diff, patch_size, target_size, top_n)
        ...
        for i in range(len(videodata)):
            ...
            for original_path, next_path in zip(original_frame_paths, next_frame_paths):
                ...
                if residual_name == 'frame_diff':
                    ...
                    residual_frag_path = process_patches(original_path, residual_name, residual, patch_size, target_size, top_n)
                    png_path, npy_path, residual_frag_npy = get_deep_feature(network_name, video_name, residual_frag_path, qp, layer_name)
                    ...
                    residual_of_frag_path = process_patches(original_path, residual_name, residual_of_rgb, patch_size, target_size, top_n)
                    png_path, npy_path, residual_frag_npy = get_deep_feature(network_name, video_name, residual_of_frag_path, qp, layer_name)
                ...
                # feature combined
                all_frame_activations_residual.append(residual_frag_npy)
            ...
            if residual_name == 'frame_diff':
                output_npy_path = f'../features_residual_frag/{network_name}/{layer_name}/{dataset_name}/{compressed_type}/'
            elif residual_name == 'optical_flow':
                output_npy_path = f'../features_residual_of_frag/{network_name}/{layer_name}/{dataset_name}/{compressed_type}/'
