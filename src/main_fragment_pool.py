...
def load_metadata(video_type, compressed_type, codec_name=None):
    ...
    if 'original' in compressed_type:
        ...
        # NR: original
            ...
            # return pd.read_csv(f'../metadata/{video_type.upper()}_metadata_fragment_pool.csv')
...
def get_video_paths(network_name, video_type, compressed_type, videodata, i):
    ...
    if compressed_type == 'original':
        ...
        # NR: original
            ...
            video_name = os.path.splitext(os.path.basename(video_path))[0]
        sampled_frame_path = os.path.join(common_path, f'fragment_pool', f'video_{str(i + 1)}')
    ...
    if video_type in valid_video_types:
        for i in range(len(videodata)):
            ...
            for original_path, next_path in zip(original_frame_paths, next_frame_paths):
                ...
                png_path, npy_path, merged_frag_npy = get_deep_feature(network_name, video_name, merged_frag_path, qp, layer_name)
            ...
            averaged_frame_npy_original = process_video_feature(all_frame_activations_original, network_name)
